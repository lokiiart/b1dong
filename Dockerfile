...
ENV INSTALL_PATH /b1dong
# The name of the application is b1dong and while there
...
# having to CD into the /b1dong folder every time would be
# lame, so instead we can set the WORKDIR to be /b1dong.
#
...
# b1dong/ path inside of the Docker image.
#
# It copies it to /b1dong because of the WORKDIR being set.
#
...
# /b1dong folder inside of the Docker image.
#
